...
| `isAlert`            | `boolean`                      | Spécifie si la modale est une alerte (valeur: `true`: rôle `"alertdialog"` en cas d'actions ou rôle `"alert"` si la modale n'est qu'informative) ou non (valeur: `false`: pas besoin de rôle car la balise est `"<dialog>"`). | `false`                                                        |              |
