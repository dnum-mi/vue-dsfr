...
import '@gouvfr/dsfr/dist/core/core.module.js'  // Import des scripts globaux
...
Il faut ajouter le plugin en créant le dossier `/plugins` s’il n’existe pas et en créant dedans un fichier `vue-dsfr.js`
avec le contenu suivant (attention ce plugin sera exécuté côté serveur et côté client) :
...
Ensuite, il faut ajouter un deuxième plugin dans un fichier `vue-dsfr.client.js`, pour charger les scripts
officiels du design system de l’État seulement côté client (ils dépendents de `document` et `window`) :

```javascript
// /plugins/vue-dsfr.client.js

import '@gouvfr/dsfr/dist/core/core.module.js'  // Import des scripts globaux côté client

export default defineNuxtPlugin(() => {
  // Ce plugin ne fait rien à part s’assurer qu’il est chargé côté client
})
```
...
Pour installer les icônes embarquées par le DSFR, veuillez consulter
[la page dédiée](/story/fondamentaux-4-1-icônes-officielles--page.
